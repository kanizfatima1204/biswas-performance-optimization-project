...
`railway.json` configures Railpack to build Vite assets, set write permissions on Laravel's cache/storage directories, run migrations through a MySQL/PostgreSQL advisory lock before deploy, and health-check Laravel's `/up` endpoint. `composer.json` declares `ext-pdo_mysql` so Railpack installs the PDO MySQL driver required by Laravel's MySQL connection. Tracked `.gitignore` placeholders preserve Laravel's required writable directories in fresh source checkouts. Railway's Laravel Railpack integration supplies the PHP-FPM/Caddy web server. Set `APP_ENV=production`, `APP_DEBUG=false`, a generated `APP_KEY`, and `LOG_CHANNEL=stderr` in the Railway service variables. Connect the Railway MySQL service to the app service by adding `DB_URL=${{MySQL.MYSQL_URL}}` to the app service's Variables (replace `MySQL` with the exact database service name). Alternatively, reference its `MYSQLHOST`, `MYSQLPORT`, `MYSQLDATABASE`, `MYSQLUSER`, and `MYSQLPASSWORD` as `DB_HOST`, `DB_PORT`, `DB_DATABASE`, `DB_USERNAME`, and `DB_PASSWORD`. The Laravel MySQL config also accepts the native `MYSQL_*` names. For PostgreSQL set `DB_CONNECTION=pgsql` and `DB_URL` to the database service URL (for example `${{Postgres.DATABASE_URL}}`). The migration pre-deploy step requires a reachable database.
